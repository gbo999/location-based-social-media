    ...
    public class GeoHashQuery
    {
        public static class Utils
        {
            public static double bitsLatitude(double resolution) {
                return Math.Min(Math.Log(Constants.EARTH_MERIDIONAL_CIRCUMFERENCE 
                    / 2 / resolution) / Math.Log(2),
            ...
            public static double bitsLongitude(double resolution, double latitude) {
                ...
                return (Math.Abs(degrees) > 0) ? Math.Max(1, Math.Log(360 / degrees) 
                    / Math.Log(2)) : 1;
            }
        ...
        public static GeoHashQuery queryForGeoHash(GeoHash geohash, int bits)
        {
            ...
            string theBase = hash.Substring(0, hash.Length - 1);
            int lastValue = Base32Utils.base32CharToValue(hash[hash.Length - 1]);
            int significantBits = bits - (theBase.Length * Base32Utils.BITS_PER_BASE32_CHAR);
            ...
            String startHash = theBase + Base32Utils.valueToBase32Char(startValue);
            String endHash;

            if (endValue > 31) {
                endHash = theBase + "~";
            } else {
                endHash = theBase + Base32Utils.valueToBase32Char(endValue);
            }
        ...
        public static HashSet<GeoHashQuery> queriesAtLocation(GeoLocation location, double radius)
        {
            ...
            int geoHashPrecision = (int)(Math.Ceiling(((float)queryBits) 
                / Base32Utils.BITS_PER_BASE32_CHAR));
            ...
            GeoHash geoHashW = new GeoHash(latitude, GeoUtils.wrapLongitude(longitude 
                - longitudeDelta), geoHashPrecision);
            GeoHash geoHashE = new GeoHash(latitude, GeoUtils.wrapLongitude(longitude 
                + longitudeDelta), geoHashPrecision);

            GeoHash geoHashN = new GeoHash(latitudeNorth, longitude, geoHashPrecision);
            GeoHash geoHashNW = new GeoHash(latitudeNorth, GeoUtils.wrapLongitude(longitude 
                - longitudeDelta), geoHashPrecision);
            GeoHash geoHashNE = new GeoHash(latitudeNorth, GeoUtils.wrapLongitude(longitude 
                + longitudeDelta), geoHashPrecision);

            GeoHash geoHashS = new GeoHash(latitudeSouth, longitude, geoHashPrecision);
            GeoHash geoHashSW = new GeoHash(latitudeSouth, GeoUtils.wrapLongitude(longitude 
                - longitudeDelta), geoHashPrecision);
            GeoHash geoHashSE = new GeoHash(latitudeSouth, GeoUtils.wrapLongitude(longitude 
                + longitudeDelta), geoHashPrecision);
        ...
        public bool canJoinWith(GeoHashQuery other) {
            return this.isPrefix(other) || other.isPrefix(this) || this.isSuperQuery(other) 
        ...
        public GeoHashQuery joinWith(GeoHashQuery other) {
            if (other.isPrefix(this)) {
                ...
                throw new UnityException("Can't join these 2 queries: " + this + ", " + other);
            }
        }

        public bool containsGeoHash(GeoHash hash) {
            string hashStr = hash.getGeoHashString();
            return startValue.CompareTo(hashStr) <= 0 
                && endValue.CompareTo(hashStr) > 0;
        }
