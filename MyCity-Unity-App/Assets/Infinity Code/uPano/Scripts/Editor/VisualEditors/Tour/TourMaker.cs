...
namespace InfinityCode.uPano.Editors.VisualEditors.Tours
{
    public class TourMaker: EditorWindow
    {
        ...
        private TourItemDrawer selectedDrawer;
        private Tour tour;
        ...
        {
            item.tour = tour;
            TourItemDrawer drawer = new TourItemDrawer(item);
            drawers.Add(drawer);
        ...
        private void DrawConnections(Event e)
        {
            ...
            {
                if (item != selectedDrawer) item.DrawConnections(e);
            }
            if (selectedDrawer != null) selectedDrawer.DrawConnections(e);
        }
        ...
        private void DrawCreateTour()
        {
            ...
            EditorGUI.LabelField(new Rect(position.width / 2 - 150, position.height / 2 - 50, 300, 100), "No Tour", style);
        ...
        private void DrawHelp()
        {
            ...
            GUILayout.BeginArea(new Rect(position.width - helpWidth - 5, 20, helpWidth, helpHeight), EditorStyles.helpBox);
        ...
        private void DrawNewConnection(Event e)
        {
        ...
        {
            if (drawers != null) drawers.Clear();
            else drawers = new List<TourItemDrawer>();
        ...
        private void OnGUI()
        {
            ...
            if (hasTour)
            {
                ProcessItemsEvents(Event.current);
        ...
        {
            if (selectedDrawer != null) selectedDrawer.isSelected = false;
            selectedDrawer = drawer;
            if (drawer != null)
            {
                Selection.activeGameObject = drawer.item.gameObject;
            }
            GUI.changed = true;
        }
        ...
        private void OnStopConnection(TourItemDrawer item)
        {
            ...
            {
                item.ProcessContextMenu(Event.current);
            }
        ...
        private void ProcessEvents(Event e)
        {
            ...
            else if (e.type == EventType.MouseUp)
            {
                if (e.button == 0)
                {
                    if (!isDragged && selectedDrawer != null)
                    {
                        selectedDrawer.isSelected = false;
                        selectedDrawer = null;
        ...
        private void ProcessItemsEvents(Event e)
        {
            foreach (TourItemDrawer item in drawers)
            {
                item.ProcessEvents(e, tour.scale);
            }
        }
        ...
        private void UpdateSelectedItem()
        {
            ...
            selectedDrawer.isSelected = true;
        }
    }
}