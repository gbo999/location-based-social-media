...
namespace SocialApp
{
    public class MessageListViewController : MonoBehaviour
    {
        ...
        public void DisplayInfo(string _chatID)
        {
            ...
            AppManager.FIREBASE_CONTROLLER.GetGroupChatInfo(_chatID, _info =>
            {
                ...
                else
                {
                    ...
                    _newInfo.ChatID = AppManager.FIREBASE_CONTROLLER.GetUserMessageKey(AppManager.Instance.auth.CurrentUser.UserId, _chatID);
                    _newInfo.Type = MessageType.Messanging;
                    _newInfo.Users = new List<string>();
                    _newInfo.Users.Add(AppManager.Instance.auth.CurrentUser.UserId);
        ...
        public void AddListeners()
        {
            if (MessageInfo.Type == MessageType.Messanging)
            {
                CountRef = AppManager.FIREBASE_CONTROLLER.GetUnreadCountWithUserReferece(AppManager.FIREBASE_CONTROLLER.GetUserMessageKey(AppManager.Instance.auth.CurrentUser.UserId, CurrentUserID));
            }
            else if (MessageInfo.Type == MessageType.Group)
            {
