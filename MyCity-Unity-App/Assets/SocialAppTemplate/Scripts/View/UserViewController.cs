...
namespace SocialApp
{
    ...
    {
        [SerializeField]
        private TMP_Text FullNameLabel = default;
        
        [SerializeField]
        private TMP_Text _scoreLabel = default;
        ...
        public void DisplayInfoOfOwner(string userID)
        {
            ClearData();
            AvatarView.SetCacheTexture(CacheAvatar);
            AppManager.FIREBASE_CONTROLLER.GetUserFullName(userID, fullname =>
            {
                FullNameLabel.text = fullname;
            });
            
            AppManager.FIREBASE_CONTROLLER.GetScore(userID, score =>
            {
                _scoreLabel.text = $"{score}";
            });

            
            AvatarView.LoadAvatar(userID);            
        ...
        public void DisplayInfo(User _user)
        {
            ...
            AppManager.FIREBASE_CONTROLLER.GetScore(_user.UserID, score =>
            {
                _scoreLabel.text = $"{score}";
            });
            GetProfileImage();
