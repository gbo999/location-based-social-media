...
namespace DTT.Utils.Workflow
{
    ...
    public static class PathUtility
    {
        ...
        public static bool InDTTDirectory(string assetPath)
        {
            ...
            string rootDirectory = GetPathElementAt(assetPath, 0);

            if (rootDirectory == "Assets")
            {
                return ContainsDirectory(assetPath, "DTT");
            }
            else if (rootDirectory == "Packages")
            {
                string subDirectory = GetPathElementAt(assetPath, 1);
        ...
        public static bool IsAssetJson(string assetPath) => Path.HasExtension(assetPath)
            && InDTTDirectory(assetPath)
            && Path.GetFileName(assetPath) == "asset.json";
